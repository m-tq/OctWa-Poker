/**
 * Escrow Cryptography Module for OCT Poker
 *
 * SECURITY CRITICAL:
...
import type { EncryptedEscrowKey, EscrowWallet } from './types.js';
...
function getMasterPassword(): string {
  const masterPassword = process.env.ESCROW_MASTER_PASSWORD;

  if (!masterPassword) {
    throw new Error(
      'SECURITY ERROR: ESCROW_MASTER_PASSWORD not configured'
    );
  }

  if (process.env.NODE_ENV === 'production' && masterPassword.length < 32) {
    throw new Error(
      'SECURITY ERROR: ESCROW_MASTER_PASSWORD must be at least 32 characters in production'
    );
  }
 ...
 */
export function encryptPrivateKey(privateKey: Uint8Array, sessionId: string): EncryptedEscrowKey {
 ...
 */
export function decryptPrivateKey(encryptedData: EncryptedEscrowKey, sessionId: string): Uint8Array {
...
/**
 * Generate unique escrow wallet for a buy-in session
 */
export async function generateEscrowWallet(sessionId: string): Promise<{
  escrow: EscrowWallet;
  encryptedKey: EncryptedEscrowKey;
}> {
  ...
  do {
    ...
    if (attempts >= maxAttempts) {
      throw new Error('Failed to generate escrow wallet with valid address');
    }
  ...
  console.log(`[ESCROW] Generated wallet for session ${sessionId.slice(0, 8)}...`);
  console.log(`[ESCROW]   Address: ${octraAddress}`);
  
  return { escrow, encryptedKey };
}
 ...
 */
export function canDecryptKey(encryptedKey: EncryptedEscrowKey, sessionId: string): boolean {
  try {
