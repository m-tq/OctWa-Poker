...
export class GameManager {
  ...
  startHand(table: Table): Hand | null {
    ...
    if (activePlayers.length < 2) {
      console.log(
        `[GameManager] Cannot start hand - only ${activePlayers.length} active players with chips`,
      );
      return null;
    }
  ...
  processAction(
    ...
    if (player.status !== "active") {
      return {
        success: false,
        error: "Cannot act - player is " + player.status,
      };
    }
    ...
    switch (action) {
      ...
      case "raise": {
        ...
        const minBet =
          action === "bet" ? table.bigBlind : hand.currentBet + table.bigBlind;
  ...
  private advanceGame(table: Table): void {
    ...
    if (playersWhoCanAct.length <= 1) {
      ...
      const allBetsMatched = activePlayers.every(
        (p) => p.bet === maxBet || p.status === "all-in",
      );
    ...
    const playersToAct = activePlayers.filter((p) => {
      ...
      const hasActedThisRound = hand.actions
        .slice(roundStartIndex)
        .some((a) => a.playerId === p.id);
    ...
    if (playersToAct.length === 0) {
      ...
      hand.activePlayerIndex = this.getNextActivePlayer(
        table,
        hand.activePlayerIndex,
      );
    }
  }
  ...
  evaluateShowdown(table: Table): HandResult | null {
    ...
    const playerHands: { player: Player; evaluated: EvaluatedHand }[] =
      activePlayers.map((player) => ({
        player,
        evaluated: this.evaluateHand([
          ...player.holeCards!,
          ...hand.communityCards,
        ]),
      }));
  ...
  private evaluateFiveCards(cards: Card[]): EvaluatedHand {
    ...
    if (isFlush && isStraight && ranks[0] === 14) {
      ...
      score =
        7000000 +
        this.getGroupScore(groups, 3) * 100 +
        this.getGroupScore(groups, 2);
    } else if (isFlush) {
      rank = "flush";
      ...
      score =
        2000000 +
        this.getGroupScore(groups, 2) * 10000 +
        this.getKickerScore(ranks);
    } else {
      rank = "high-card";
  ...
  private getKickerScore(ranks: number[]): number {
    return ranks.reduce(
      (score, rank, i) => score + rank * Math.pow(15, 4 - i),
      0,
    );
  }
