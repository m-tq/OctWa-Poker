...
interface EscrowBuyInDialogProps {
  open: boolean;
  onClose: () => void;
  table: Table | null;
...
export function EscrowBuyInDialog({
  open,
  onClose,
  table,
  walletBalance,
  ...
  const handleGetQuote = async () => {
    ...
    try {
      const response = await fetch('/api/escrow/quote', {
        method: 'POST',
  ...
  return (
    <Dialog open={open} onClose={onClose} title="Buy In">
      <div className="space-y-6">
          ...
          <>
            ...
            <div className="flex justify-end gap-2 pt-4">
              ...
              {escrowEnabled ? (
                ...
                >
                  {isLoading ? 'Getting Quote...' : 'Get Escrow Quote'}
                </Button>
              ) : (
                <Button
        ...
        {/* Step 2: Show quote and escrow address */}
        {step === 'quote' && quote && (
          <>
            <div className="bg-primary/10 border border-primary p-4 text-center">
              ...
              <p className="text-sm text-muted mt-2">to the escrow address below</p>
            </div>

            <div className="bg-secondary p-3">
              <label className="text-xs text-muted block mb-1">Escrow Address</label>
              <div className="flex items-center gap-2">
                <code className="text-xs text-foreground break-all flex-1">{quote.escrowAddress}</code>
                <Button
                  variant="secondary"
                  size="sm"
                  onClick={() => copyToClipboard(quote.escrowAddress)}
                >
                  Copy
                </Button>
              </div>
